...
import ValueCard from "@/component/about/ValueCard";
import { Award, Heart, Shield, Truck, Users } from "lucide-react";
import { FaStar } from "react-icons/fa";
import TeamCard from "@/component/about/TeamCard";
      ...
      <article className="lg:px-4 px-0 mx-auto space-y-6">
          ...
          <article className="flex justify-center">
            {[1, 2, 3, 4, 5].map((item) => (
              <div key={item} className="mt-12">
                <FaStar
                  className="text-[#00AD7C] hover:text-green-500 m-4 animate-pulse"
                  size={32}
                />
              </div>
            ))}
        ...
        <section className="flex flex-col items-center justify-center text-center bg-gray-200 rounded-md  py-12 my-12">
          ...
          <div className=" block space-y-4 lg:flex lg:space-x-12  mt-8">
            <button
              onClick={() => route.push("/catalog")}
              className="font-medium cursor-pointer text-xl flex items-center justify-center gap-2 w-[350.54px] rounded-[8px]  px-4 py-3 text-gray-100 border-2 border-gray-300 bg-green-600 hover:bg-green-800 hover:text-white transition"
            >
              <span>Browse Catalog</span>
            </button>
            <button
              onClick={() => route.push("/categories")}
              className="font-medium cursor-pointer text-xl flex items-center justify-center gap-2 w-[350.54px] rounded-[8px]  px-4 py-3 text-gray-800 border-2 border-gray-300 bg-gray-50 hover:bg-green-800 hover:text-white transition"
            >
